...
const didSelectSuggestion = ref(false)
...
const selectSuggestion = (item: UserSearchResult): void => {
  // Guardamos el usuario seleccionado (este es el que viene de instagrapi)
  dataModel.value.username = item.suffix
  dataModel.value.user = item
  profileResult.value = item

  // Reset de posts/selecciones al cambiar de usuario
  postsModel.value = { posts: [], total: 0, skip: 0, limit: 12 }
  dataModel.value.selected_posts = []
...
const handleUsernameBlur = async (): Promise<void> => {
  const selected = didSelectSuggestion.value
  didSelectSuggestion.value = false
  setTimeout(() => {
    showSuggestions.value = false
  }, 150)
  if (selected) return
                    ...
                    <div v-else-if="searchSuggestions.length" class="space-y-1">
                      ...
                      >
                        <img
                          ...
                          :src="`${API_BASE}/api/proxy/image?url=${encodeURIComponent(item.avatar.src)}`"
